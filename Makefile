...
start:
	...
	docker run -d --volumes-from workspace -w /app -p 1042:1042 -p 35729:35729 $(DOCKER_IMAGE) npm start
...
greet:
	...
	@docker run --volumes-from workspace -w /app --entrypoint ./resources/example.sh -e NAME="${NAME}" $(DOCKER_IMAGE)
